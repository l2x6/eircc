...
public final class GdBuilder {
    ...
    public static GridData copyData(GridData data) {
        GridData newData = new GridData(data.horizontalAlignment, data.verticalAlignment,
                data.grabExcessHorizontalSpace, data.grabExcessVerticalSpace, data.horizontalSpan, data.verticalSpan);
    ...
    /**
     * Returns a GridDataFactory initialized with heuristicly generated defaults
     * for the given control. To be precise, this method picks the default
     * values that GridLayoutFactory.generateLayout would have assigned to the
     * control. Does not attach GridData to the control. Callers must
     * additionally call applyTo(theControl) if they wish to use the generated
     * values.
     *
     * <p>
     * This method is intended for situations where generateLayout is generating
     * layout data for a particular control that is not quite right for the
     * desired layout. This allows callers to start with the generated values
     * and tweak one or two settings before applying the GridData to the
     * control.
     * </p>
     *
     * @see GlFactory#generateLayout(org.eclipse.swt.widgets.Composite)
     * @param control
     * @return a GridLayoutFactory initialized with defaults that
     *         GridLayoutFactory would have
     * @since 3.3
     */
    public static GdBuilder defaults(Control control) {
        GridDataFactory f = GridDataFactory.defaultsFor(control);
        return new GdBuilder(f.create(), control);
    }
     ...
     * @param control
     */
    private GdBuilder(GridData data, Control control) {
        super();
        this.data = data;
     ...
     *
     * @param hAlign
     *            horizontal alignment. One of SWT.BEGINNING, SWT.CENTER,
     *            SWT.END, or SWT.FILL.
     * @param vAlign
     *            vertical alignment. One of SWT.BEGINNING, SWT.CENTER, SWT.END,
     *            or SWT.FILL.
     * @return this
     */
    public GdBuilder align(int hAlign, int vAlign) {
    ...
    public void apply() {
        this.control.setLayoutData(create());
    }

    /**
     * Sets the layout data on the given control. Creates a new GridData
     ...
     * @return this
     */
    public GdBuilder exclude(boolean shouldExclude) {
        data.exclude = shouldExclude;
        return this;
    }

    /**
     * Determines whether extra horizontal or vertical space should be allocated
     * to this control's column when the layout resizes. If any control in the
     * column is set to grab horizontal then the whole column will grab
     * horizontal space. If any control in the row is set to grab vertical then
     ...
     * @return this
     */
    public GdBuilder grab(boolean horizontal, boolean vertical) {
     ...
     * @return this
     */
    public GdBuilder hint(int xHint, int yHint) {
     ...
     * @return this
     */
    public GdBuilder hint(Point hint) {
        data.widthHint = hint.x;
        data.heightHint = hint.y;
        return this;
    }

    public GdBuilder hSpan(int hSpan) {
        data.horizontalSpan = hSpan;
        return this;
    }

    /**
     * Sets the indent of the control within the cell. Moves the position of the
     * control by the given number of pixels. Positive values move toward the
     * lower-right, negative values move toward the upper-left.
     *
     * @param hIndent
     *            distance to move to the right (negative values move left)
     * @param vIndent
     *            distance to move down (negative values move up)
     * @return this
     */
    public GdBuilder indent(int hIndent, int vIndent) {
     ...
     *
     * @param indent
     *            offset to move the control
     * @return this
     */
    public GdBuilder indent(Point indent) {
     ...
     * to shrink below this size. Note: GridLayout treats a minimum size of 0 as
     * an undocumented special value, so the smallest possible minimum size is a
     * size of 1. A minimum size of SWT.DEFAULT indicates that the result of
     * computeSize(int, int, boolean) should be used as the control's minimum
     * size.
     *
     *
     * @param minX
     *            minimum a value of 1 or more is a horizontal size of the
     *            control (pixels). SWT.DEFAULT indicates that the control's
     *            preferred size should be used. A size of 0 has special
     *            semantics defined by GridLayout.
     * @param minY
     *            minimum a value of 1 or more is a vertical size of the control
     *            (pixels). SWT.DEFAULT indicates that the control's preferred
     *            size should be used. A size of 0 has special semantics defined
     *            by GridLayout.
     * @return this
     */
    public GdBuilder minSize(int minX, int minY) {
     ...
     * @return this
     */
    public GdBuilder minSize(Point min) {
    ...
    /**
     * Sets the GridData span. The span controls how many cells are filled by
     * the control.
     *
     * @param hSpan
     ...
     * @return this
     */
    public GdBuilder span(int hSpan, int vSpan) {
        data.horizontalSpan = hSpan;
     ...
     *
     * @param span
     *            the new span. The x coordinate indicates the number of columns
     *            spanned, and the y coordinate indicates the number of rows.
     * @return this
     */
    public GdBuilder span(Point span) {
        data.horizontalSpan = span.x;
        data.verticalSpan = span.y;
        return this;
    }

    public GdBuilder vSpan(int vSpan) {
        data.verticalSpan = vSpan;
        return this;
    }
